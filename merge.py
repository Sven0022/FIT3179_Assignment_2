...
# Sort countries alphabetically to assign consistent positions
unique_countries = sorted(df['Reference area'].unique())

# Define the side length of each hexagon (assume side length = 1 unit for simplicity)
side_length = 1

# Calculate width and height of a hexagon for positioning
col_width =0.75* math.sqrt(3) * side_length  # Use the correct horizontal distance between hexagon centers
row_height = 0.55 * side_length        # Slightly smaller vertical distance between hexagon centers

# Initialize lists to store the positions
x_positions = []
y_positions = []

# Iterate through the countries and assign positions
hexagons_per_row = 4 # Fewer columns, more rows

# Iterate through the countries and assign positions
for i, country in enumerate(unique_countries):
    row = i // hexagons_per_row  # Adjust for more rows, fewer columns
    col = i % hexagons_per_row
    x = col * col_width
    y = row * row_height
    
    # Apply offset for every other row to create a honeycomb pattern
    if row % 2 == 1:
        x += col_width / 2
...
# Map x_position and y_position to each country
df['x_position'] = df['Reference area'].map(dict(zip(unique_countries, x_positions)))
df['y_position'] = df['Reference area'].map(dict(zip(unique_countries, y_positions)))

# Save the updated dataset with hexagonal layout positions
df.to_csv('forest_with_honeycomb_positions.csv', index=False)
